...
| `POST /api/rag/upload` | 上传文件（PDF/DOCX/TXT等），自动解析文本并入库 |
| `GET /api/rag/search` | 检索相似文档 (topK, similarityThreshold) |
| `POST /api/rag/ask` | RAG 问答 |
| `GET /api/rag/verify` | 检索校验：返回初筛+重排+评分详情 |
| `POST /api/rag/init` | 初始化测试文档 |
...
├── parser/
│   └── DocumentParser.java          # Tika 文件解析 (PDF/DOCX/TXT等)
├── service/
...
- **文档解析**: Apache Tika (tika-core + tika-parsers)，支持 PDF/DOCX/DOC/TXT/HTML/PPT 等格式
